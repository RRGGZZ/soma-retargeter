...
class HumanToRobotScaler:
    ...
    def __init__(self, skeleton: Skeleton, human_height, config_file):
        ...
        # Resolve toe effectors with backward-compatible fallbacks.
        # Priority:
        #   1) Toe / ToeBase (legacy)
        #   2) LEFT_ANKLE / RIGHT_ANKLE (npz-kai style)
        #   3) LEFT_FOOT / RIGHT_FOOT
        if "LeftToe" in joint_offsets:
            joint_offsets["LeftToeBase"] = joint_offsets["LeftToe"]
        elif "LeftToeBase" not in joint_offsets:
            for fallback_name in ("LEFT_ANKLE", "LeftAnkle", "LEFT_FOOT", "LeftFoot"):
                if fallback_name in joint_offsets:
                    joint_offsets["LeftToeBase"] = joint_offsets[fallback_name]
                    break
            if "LeftToeBase" not in joint_offsets:
                raise KeyError(
                    "Missing left toe offset: expected one of "
                    "'LeftToe', 'LeftToeBase', 'LEFT_ANKLE', 'LeftAnkle', 'LEFT_FOOT', 'LeftFoot'."
                )

        if "RightToe" in joint_offsets:
            joint_offsets["RightToeBase"] = joint_offsets["RightToe"]
        elif "RightToeBase" not in joint_offsets:
            for fallback_name in ("RIGHT_ANKLE", "RightAnkle", "RIGHT_FOOT", "RightFoot"):
                if fallback_name in joint_offsets:
                    joint_offsets["RightToeBase"] = joint_offsets[fallback_name]
                    break
            if "RightToeBase" not in joint_offsets:
                raise KeyError(
                    "Missing right toe offset: expected one of "
                    "'RightToe', 'RightToeBase', 'RIGHT_ANKLE', 'RightAnkle', 'RIGHT_FOOT', 'RightFoot'."
                )
        ...
        self.mapped_joint_scale_modes = wp.array(
            [
                1 if name in ("LeftForeArm", "LeftHand")
                else 2 if name in ("RightForeArm", "RightHand")
                else 0
                for name in self.mapped_joints
            ],
            dtype=wp.int32,
        )
        self.mapped_joint_arm_roles = wp.array(
            [
                1 if name == "LeftArm"
                else 2 if name == "RightArm"
                else 0
                for name in self.mapped_joints
            ],
            dtype=wp.int32,
    ...
    def compute_effectors_from_skeleton(self, skeleton_instance: SkeletonInstance, scale_animation: bool):
        ...
        def compute_scaled_effectors_kernel(
            ...
            in_mapped_joint_scale_modes : wp.array(dtype=wp.int32),
            in_mapped_joint_arm_roles : wp.array(dtype=wp.int32),
            ...
            HumanToRobotScaler.wp_compute_scaled_effectors(
                ...
                in_mapped_joint_scales, in_mapped_joint_offsets, in_mapped_joint_scale_modes, in_mapped_joint_arm_roles, in_scale_animation, out_result)
        ...
        wp.launch(
            ...
            inputs=[
                ...
                self.mapped_joint_scale_modes,
                self.mapped_joint_arm_roles,
                scale_animation
            ],
    ...
    def compute_effectors_from_buffer(self, animation_buffer: AnimationBuffer, scale_animation: bool, xform: wp.transform = wp.transform_identity()):
        ...
        def batched_compute_scaled_effectors_2d_kernel(
            ...
            HumanToRobotScaler.wp_compute_scaled_effectors(
               ...
               in_mapped_joint_scales, in_mapped_joint_offsets, in_mapped_joint_scale_modes, in_mapped_joint_arm_roles, in_scale_animation, out_result[frame_idx])
    ...
    def wp_compute_scaled_effectors(
        ...
        root_body_q = wp.normalize(wp.mul(in_global_pose[in_mapped_joint_indices[0]].q, in_mapped_joint_offsets[0].q))
        ...
        left_arm_t = root_t
        right_arm_t = root_t
        scaled_left_arm_t = scaled_root_t
        scaled_right_arm_t = scaled_root_t
        ...
        for i in range(in_num_mapped_joints):
            ...
            scale_mode = in_mapped_joint_scale_modes[i]

            anchor_t = root_t
            scaled_anchor_t = scaled_root_t
            if scale_mode == 1:
                anchor_t = left_arm_t
                scaled_anchor_t = scaled_left_arm_t
            elif scale_mode == 2:
                anchor_t = right_arm_t
                scaled_anchor_t = scaled_right_arm_t

            scaled_joint_t = scaled_anchor_t + wp.cw_mul((pose_tx.p - anchor_t), scale)
            arm_role = in_mapped_joint_arm_roles[i]
            if arm_role == 1:
                left_arm_t = pose_tx.p
                scaled_left_arm_t = scaled_joint_t
            elif arm_role == 2:
                right_arm_t = pose_tx.p
                scaled_right_arm_t = scaled_joint_t

            q = wp.mul(pose_tx.q, offset_tx.q)
            t = scaled_joint_t + wp.quat_rotate(root_body_q, offset_tx.p)
