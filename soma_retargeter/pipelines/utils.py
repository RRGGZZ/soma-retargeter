...
def get_source_model_mesh(source: SourceType, skeleton) -> dict:
    ...
    if source == SourceType.OURS:
        return usd_utils.load_skeletal_mesh_from_usd(
            str(io_utils.get_config_file('soma', 'soma_base_skel_minimal.usd')),
            skeleton,
            '/OUTPUT/c_geometry_grp',
            '/OUTPUT/c_skeleton_grp/Root')
    if source == SourceType.NPZ:
        return usd_utils.load_skeletal_mesh_from_usd(
            str(io_utils.get_config_file('soma', 'soma_base_skel_minimal.usd')),
            skeleton,
            '/OUTPUT/c_geometry_grp',
...
def get_retargeter_config(source: SourceType, target: TargetType) -> dict:
    ...
    if target == TargetType.UNITREE_G1:
        ...
        elif source == SourceType.OURS:
            filename = 'ours_to_g1_retargeter_config.json'
        else:
    ...
    if target == TargetType.ADAM_PRO:
        if source == SourceType.SOMA:
            filename = 'soma_to_adam_retargeter_config.json'
        elif source == SourceType.OURS:
            filename = 'ours_to_adam_retargeter_config.json'
        else:
            raise ValueError(f"Unknown source type [{source}] for target [{target}].")
        return io_utils.load_json(io_utils.get_config_file('adam_pro', filename))

    if target == TargetType.KAI:
        if source == SourceType.SOMA:
            filename = 'soma_to_kai_retargeter_config.json'
        elif source == SourceType.NPZ:
            filename = 'npz_to_kai_retargeter_config.json'
        else:
            raise ValueError(f"Unknown source type [{source}] for target [{target}].")
        return io_utils.load_json(io_utils.get_config_file('kai', filename))
