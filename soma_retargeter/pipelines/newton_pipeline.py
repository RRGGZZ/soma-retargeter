...
class NewtonPipeline:
    ...
    def __init__(self, skeleton: Skeleton, source_type='soma', robot_type='unitree_g1', retarget_config: dict = None):
        ...
        if self.target_type in (
            ...
            pipeline_utils.TargetType.ADAM_PRO,
            pipeline_utils.TargetType.KAI,
        ):
            import pathlib
            self.robot_builder = newton.ModelBuilder()
            if self.target_type == pipeline_utils.TargetType.UNITREE_G1:
                mjcf_path = str(pathlib.Path(__file__).parent.parent.parent / "assets/robot/unitree_g1/g1_mocap_29dof.xml")
            elif self.target_type == pipeline_utils.TargetType.ADAM_PRO:
                mjcf_path = str(pathlib.Path(__file__).parent.parent.parent / "assets/robot/adam_pro/adam_pro.xml")
            else:
                mjcf_path = str(pathlib.Path(__file__).parent.parent.parent / "assets/robot/kai/kai.xml")
            ...
            def find_mapped_joint_index(candidates):
                for name in candidates:
                    if name in self.mapped_joints:
                        return self.mapped_joints.index(name)
                return -1

            left_foot_idx = find_mapped_joint_index(["LeftFoot", "LEFT_FOOT", "LeftAnkle", "LEFT_ANKLE"])
            right_foot_idx = find_mapped_joint_index(["RightFoot", "RIGHT_FOOT", "RightAnkle", "RIGHT_ANKLE"])
            if left_foot_idx == -1 or right_foot_idx == -1:
                raise ValueError(
                    "[ERROR]: Could not determine foot effectors from ik_map. "
                    "Expected one of Left/RightFoot or LEFT/RIGHT_FOOT or *_ANKLE."
                )
            self.feet_effector_indices = [left_foot_idx, right_foot_idx]
            ...
            if (retargeter_config['initialization_pose']):
                init_skel, init_anim = bvh_utils.load_bvh(
                    ...
                    source_type=pipeline_utils.get_source_str_from_type(self.source_type),
                )
            ...
            raise ValueError(f"Unsupported robot type: {self.target_type}.")
    ...
    def add_input_motions(self, buffers: list[AnimationBuffer], offsets: list[wp.transform], scale_animation: bool):
        ...
        for i in trange(len(buffers), desc="[INFO] Converting Motions for Newton"):
            ...
            self.input_full_targets.append(buffer_effectors)
    ...
    def get_scaled_ik_targets(self, trim_initialization_frames: bool = True):
        """
        Return IK targets for each input motion in mapped IK order.

        Targets are exported in IK-map order (matching ``self.mapped_joints``),
        where each transform is represented as ``[tx, ty, tz, qx, qy, qz, qw]``.
        This excludes scaler effectors that are not mapped to robot IK bodies.

        Args:
            trim_initialization_frames: If True, drop initialization and
                stabilization frames to match exported retargeted motion length.

        Returns:
            tuple[list[np.ndarray], list[str]]: A tuple of
            ``(targets_per_motion, mapped_joint_names)``.
        """
        # Use IK-mapped targets only (same targets used by the solver), so
        # exported PKLs do not contain unmapped effectors.
        source_targets = self.input_targets
        joint_names = list(self.mapped_joints)

        if not trim_initialization_frames:
            return list(source_targets), joint_names

        num_frames_to_remove = self.num_initialization_frames + self.num_stabilization_frames
        trimmed_targets = []
        for target in source_targets:
            trimmed_targets.append(target[num_frames_to_remove:])
        return trimmed_targets, joint_names
